...
class BaseModelMemcached {
    ...
    public function __construct($mcName = '', $servers = '', $native = true, $compress = true, $tcp_nodelay = true) {
        ...
        if (isset(self::$memcached[$mcKey]) && self::$memcached[$mcKey] instanceof Memcached) {
            ...
            defined('DEBUG') && BaseModelCommon::debug(implode(',', $serverInfo), 'mcd_servers');
    ...
    public function __call($name, $args) {
        ...
        defined('DEBUG') && BaseModelCommon::debug($ret, "mcd_{$name}_" . implode('_', $args));
        return $ret;
    }
    ...
    public function set($key, $value, $time = 0) {
        ...
        defined('DEBUG') && BaseModelCommon::debug($value, "mcd_set({$key}),ttl({$time})");
        return $ret;
    }
    ...
    public function setByKey($server_key, $key, $value, $time = 0) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($value, "mcd_setByKey({$key}),server_key({$server_key}), ttl({$time})");
        return $ret;
    }
    ...
    public function setMulti(array $dataArr, $time = 0) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($dataArr, "mcd_setMulti,ttl({$time})");
        return $ret;
    }
    ...
    public function setMultiByKey($server_key, array $dataArr, $time = 0) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($dataArr, "mcd_setMultiByKey,server_key({$server_key}),ttl({$time})");
        return $ret;
    }
    ...
    public function get($key, $cache_cb = NULL, &$cas_token = NULL, $lockTime = 3) {
            ...
            if($this->native) {
                ...
                if($ctlResultCode !== Memcached::RES_SUCCESS || $this->lastResultCode !== Memcached::RES_SUCCESS) {
                    if($this->_getLock($key, $lockTime)) {
                        ...
                        defined('DEBUG') && BaseModelCommon::debug(false, "mcd_get_lock({$key})");
                        $redo = 3;
                        $do = 0;
                        do {
                            usleep(1000*100);
                        ...
                        if($this->mcd->getResultCode() === Memcached::RES_SUCCESS) {
                            defined('DEBUG') && BaseModelCommon::debug($do - 1, "mcd_get_waitNewData({$key})");
                            $ret = $this->mcd->get($key, NULL, $cas_token);
                            $this->lastResultCode = $this->mcd->getResultCode();
                            $this->lastResultMessage = $this->mcd->getResultMessage();
                            $calls++;
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($ret, "mcd_get({$key})");
        return $ret;
    }
    ...
    public function getByKey($server_key, $key, $cache_cb = NULL, &$cas_token = NULL, $lockTime = 3) {
            ...
            if($this->native) {
                ...
                if($ctlResultCode !== Memcached::RES_SUCCESS || $this->lastResultCode !== Memcached::RES_SUCCESS) {
                    if($this->_getLock($key, $lockTime, $server_key)) {
                        ...
                        defined('DEBUG') && BaseModelCommon::debug(false, "mcd_get_lock({$key}, {$server_key})");
                        $redo = 3;
                        do {
                            usleep(100*1000);
                        ...
                        $calls += $do - 1;
                        if($this->mcd->getResultCode() === Memcached::RES_SUCCESS) {
                            defined('DEBUG') && BaseModelCommon::debug(3 - $redo, "mcd_getByKey_waitNewData({$key}, {$server_key})");
                            $ret = $this->mcd->getByKey($server_key, $key, NULL, $cas_token);
                            $this->lastResultCode = $this->mcd->getResultCode();
                            $this->lastResultMessage = $this->mcd->getResultMessage();
                            $calls++;
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($ret, "mcd_getByKey({$key})");
        return $ret;
    }
    ...
    public function getMulti(array $keys, &$cas_tokens = NULL, $flags = NULL, $lockTime = 3) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($ret, "mcd_getMulti");
        return $ret;
    }
    ...
    public function add($key, $value, $time = 0) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($value, "mcd_add({$key}),ttl({$time})");
        return $ret;
    }
    ...
    public function addByKey($server_key, $key, $value, $time = 0) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($value, "mcd_addByKey({$key}),ttl({$time})");
        return $ret;
    }
    ...
    public function append($key, $value) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($value, "mcd_append({$key})");
        return $ret;
    }
    ...
    public function appendByKey($server_key, $key, $value) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($value, "mcd_appendByKey({$key},{$server_key})");
        return $ret;
    }
    ...
    public function prepend($key, $value) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($value, "mcd_prepend({$key})");
        return $ret;
    }
    ...
    public function prependByKey($server_key, $key, $value) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($value, "mcd_prependByKey({$key}, $server_key)");
        return $ret;
    }
    ...
    public function increment($key, $offset = 1) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($ret, "mcd_increment({$key})");
        return $ret;
    }
    ...
    public function incrementByKey($server_key, $key, $offset = 1, $initial_value = 0, $expiry = 0) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($ret, "mcd_incrementByKey({$key}),({$server_key})");
        return $ret;
    }
    ...
    public function decrement($key, $incre = 1) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug($ret, "mcd_decrement({$key})");
        return $ret;
    }
    ...
    public function delete($key, $time = 0) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug(($ret ? 'TRUE' : 'FALSE'), "mcd_delete({$key})({$time})");
        return $ret;
    }
    ...
    public function deleteByKey($server_key, $key, $time = 0) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug(($ret ? 'TRUE' : 'FALSE'), "mcd_deleteByKey({$key})({$server_key})({$time})");
        return $ret;
    }
    ...
    public function deleteMulti(array $keys, $time = 0) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug(($ret ? 'TRUE' : 'FALSE'), "mcd_deleteMulti");
        return $ret;
    }
    ...
    public function deleteMultiByKey($server_key, array $keys, $time = 0) {
        ...
        $this->_checkStats(__FUNCTION__, $calls);
        defined('DEBUG') && BaseModelCommon::debug(($ret ? 'TRUE' : 'FALSE'), "mcd_deleteMultiByKey({$server_key})");
        return $ret;
    }
    ...
    private function _getLock($key, $lockTime = 3, $server_key = NULL) {
        ...
        defined('DEBUG') && BaseModelCommon::debug(intval($ret), "mcd_lock_time({$key})({$lockTime})({$server_key})");
        return $ret;
    }
    ...
    private function _checkConnection() {
        $timeout = defined('MCCONNECT_TIMEOUT') ? MCCONNECT_TIMEOUT : 0.5;
        ...
        if($runTime > $timeout*1000) {
            ...
            defined('DEBUG') && BaseModelCommon::debug('[errro code] ' . $errno. ' [errro msg] ' . $errormsg . ' [详细说明]:http://wiki.intra.sina.com.cn/display/dagger/'.$errno, 'mc_error');
    ...
    private function _checkStats($function, $times = 0, $native = false) {
        ...
        defined('DEBUG') && BaseModelCommon::debug("[errro code] {$errno} [errro msg] {$error} [详细说明]:http://wiki.intra.sina.com.cn/display/dagger/{$errno}", 'request_error');
